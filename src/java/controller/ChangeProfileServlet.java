...
public class ChangeProfileServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(user==null){
            ...
            User userNew =  new User(user.getUserID(), name, address, email, phone, u, p, 2);
            d.changeProfile(userNew);
            HttpSession session = request.getSession();
            request.setAttribute("account", userNew);
            request.getRequestDispatcher("listtrendproduct?cid=1").forward(request, response);
        }
